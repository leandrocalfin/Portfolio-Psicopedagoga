  ...
  return (
    <section id="sobre-mi" className="relative overflow-hidden scroll-mt-20">
      ...
      <div className="max-w-6xl mx-auto px-5 pt-14 grid md:grid-cols-2 gap-10 items-center relative">
        ...
        <div className="relative w-full max-w-[290px] md:max-w-[520px] mx-auto md:mx-0 md:w-full pr-2 pb-8 md:pr-3 md:pb-10">
          {sobreMi.imagen && (
            <img src={sobreMi.imagen} alt="Psicopedagoga" className="w-full h-[230px] md:h-[500px] object-cover object-top rounded-[24px] shadow-[0_25px_60px_-15px_rgba(61,52,99,0.55),0_18px_35px_-18px_rgba(139,118,201,0.65)]" />
          )}
          {sobreMi.imagen && (
            <img
              src="/libroES.png"
              ...
              className="absolute bottom-0 right-0 w-[36%] md:w-[34%] h-auto object-contain rounded-[16px] border-[5px] md:border-[6px] border-lila-200 bg-white shadow-xl cursor-pointer hover:scale-[1.02] transition"
            />
          )}
        </div>
        <div>
        ...
        <div className="fixed inset-0 z-[60] bg-black/80 grid place-items-center p-4" onClick={() => setLibroModal(false)}>
          ...
          <div className="relative max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white rounded-[24px] overflow-hidden">
              <button onClick={() => setLibroIdx((i) => (i - 1 + 10) % 10)} className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 text-lila-900 grid place-items-center shadow-lg hover:bg-white transition z-10" aria-label="Anterior"><ChevronDown size={24} className="-rotate-90" /></button>
              <img src={`/libro${libroIdx + 1}.png`} alt={`Libro ${libroIdx + 1}`} className="w-full max-h-[75vh] object-contain" />
              <button onClick={() => setLibroIdx((i) => (i + 1) % 10)} className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 text-lila-900 grid place-items-center shadow-lg hover:bg-white transition z-10" aria-label="Siguiente"><ChevronDown size={24} className="rotate-90" /></button>
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
                {Array.from({ length: 10 }).map((_, i) => (
                  <button key={i} onClick={() => setLibroIdx(i)} className={`h-2 rounded-full transition-all ${i === libroIdx ? "w-6 bg-lila-700" : "w-2 bg-white/80"}`} aria-label={`Ver imagen ${i + 1}`} />
                ))}
              </div>
            </div>
          </div>
...
function FormacionFromAPI({ certificados }) {
  ...
  return (
    <section id="formacion" className="max-w-6xl mx-auto px-5 py-14 scroll-mt-20 relative z-10">
      ...
      <div className="mt-8 max-w-3xl mx-auto grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
        {certificados.map((c, i) => (
          <button key={c._id || i} onClick={() => setSel(i)} className="bg-white rounded-2xl overflow-hidden border border-lila-100 shadow-[0_18px_40px_-16px_rgba(95,75,158,0.45)] hover:shadow-[0_22px_48px_-14px_rgba(95,75,158,0.55)] hover:-translate-y-1 transition text-left">
