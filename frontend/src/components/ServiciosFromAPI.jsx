...
export function ServiciosFromAPI() {
  ...
  const totalItems = servicios.length + 1; // +1 for the CTA card

  const [setRef, visibleIndices] = useStaggeredReveal(totalItems, { threshold: 0.08, rootMargin: "0px 0px -30px 0px" });
        ...
        <div className="mt-8 md:mt-10 grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5 justify-center">
          {servicios.map((s, i) => {
            ...
            return (
              ...
              >
                <div className="bg-white rounded-2xl p-4 md:p-7 text-center card-shadow hover:-translate-y-1 transition">
          ...
          <div
            ref={setRef(servicios.length)}
            className={`transition-all duration-700 ease-out ${visibleIndices.has(servicios.length) ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-6 scale-95"}`}
          >
